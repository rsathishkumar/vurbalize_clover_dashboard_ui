...
var line_index = 0;
const MultiListSearchText = (props) => {
    ...
    useEffect(() => {
        ...
        line_index = props.currentValues.length;
    }, [])
    ...
    const handleAddNew = () => {
        const newValues = [...textValues];
        if (currentTextValue != '') {
            newValues[line_index] = currentTextValue;
            setTextValues(newValues);
            setCurrentTextValue('');
            line_index++;
        }
    ...
    async function submitAllValues() {
        const newValues = [...textValues];
        if (currentTextValue !== '') {
          newValues[newValues.length] = currentTextValue;
        }
    ...
    return (
        <div className="absolute bg-white rounded p-3 ml-2 left-full top-0 shadow-6xl" key={props.id}>
            ...
            />
            <button onClick={handleAddNew} className="rounded-full bg-green-900 w-[24px] h-[24px] mx-2">+</button>
            </div>
            <button 
...
export default MultiListSearchText;